defmodule StormstoutWeb.WalletLive do
  @moduledoc false
  ...
  def render(assigns) do
    ...
    <div class="my-2 space-x-4 max-w-[80rem] mx-auto">
      <.modal id="wallet-modal-2" show={true} class="min-w-[25%]" navigate={Routes.profile_path(@socket, :index)}>
        <%= live_render(@socket, StormstoutWeb.WalletButtonLive, id: "connect", session: %{"id" => "wallet-connect", "text" => "Log in with Petra Wallet"}) %>
      </.modal>
    </div>
    """
  end
