defmodule Stormstout.ImportTest do
  @moduledoc false

  use Stormstout.DataCase
  ...
  test "Import.run()" do
    ...
    {:ok, %{orders: result_orders, events: result_events}} = Stormstout.Import.run(import_list)

    assert length(result_events) == 1
    assert length(result_orders) == 1

    {:ok, result} = Stormstout.Import.run(import_list)
